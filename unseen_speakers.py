...
from utils import normalize, InputsGenerator, generate_features_for_new_file
...
class MultithreadsInference:
    def __init__(self, filename, audio_reader, speakers, num_threads=cpu_count()):
    ...
    def inference(self, filename, audio_reader, speaker):
        sp1_feat = generate_features_for_new_file(filename)
        sp2_feat = generate_features_for_unseen_speakers(audio_reader, target_speaker=speaker)
    ...
    def run(self):
        logger.info('Using {} threads.'.format(self.num_threads))
        result = self.pool.starmap(self.inference, zip(repeat(self.filename), repeat(self.audio_reader), self.speakers))
